...
from tao_triton.python.postprocessing.re_identification_postprocessor import (
    ReIdentificationPostprocessor,
)
...
def request_generator(
    batched_image_data, input_name, output_name, dtype, protocol, num_classes=0
):
    ...
    if isinstance(input_name, list):
        for i in range(len(input_name)):
            ...
            inputs = [
                client.InferInput(input_name[i], batched_image_data[i].shape, dtype)
            ]
    ...
    outputs = [
        client.InferRequestedOutput(out_name, class_count=num_classes)
...
def main():
    ...
    if FLAGS.mode.lower() == "pose_classification":
        ...
        if os.path.isdir(FLAGS.image_filename):
            ...
            if FLAGS.mode.lower() == "re_identification":
                ...
                for f in os.listdir(FLAGS.test_dir):
                    if os.path.isfile(
                        os.path.join(FLAGS.test_dir, f)
                    ) and os.path.splitext(f)[-1] in [".jpg", ".jpeg", ".png"]:
                        img_path = os.path.join(FLAGS.test_dir, f)
                        pid, _ = map(int, pattern.search(img_path).groups())
                        if pid == -1:
                            continue  # junk images are ignored
                        frame = Frame(
                            img_path, triton_model.data_format, npdtype, target_shape
                        )
        ...
        # The input is an image.
            ...
            frames = [
                Frame(
                    os.path.join(FLAGS.image_filename),
    ...
    with tqdm(total=pbar_total) as pbar:
        while not last_request:
            ...
            try:
                req_gen_args = [
                    batched_image_data,
                    triton_model.input_names,
                    triton_model.output_names,
                    triton_model.triton_dtype,
                ...
                # logger.info("Response", len(responses),"Requests", len(async_requests))
            except InferenceServerException as exc:
    ...
    logger.info((responses[-1].as_numpy("fc_pred"))[-1])
    # logger.info(
    ...
    #         else:
    #             postprocessor.apply(response, this_id, render=True)
    #         processed_request += 1
    #         pbar.update(FLAGS.batch_size)
